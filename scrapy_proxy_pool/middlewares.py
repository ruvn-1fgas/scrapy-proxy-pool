...
class ProxyPoolMiddleware(object):
    ...
    * ``PROXY_POOL_PAGE_RETRY_TIMES`` - a number of times to retry
    ...
    def __init__(self, proxy_pool, refresh_interval, logstats_interval, stop_if_no_proxies,
                 max_proxies_to_try, try_with_host):
    ...
    def from_crawler(cls, crawler):
        ...
        mw = cls(
            proxy_pool=s.getlist('PROXY_POOL_URLS', []),
    ...
    def get_proxy_slot(self, proxy):
        ...
        """
        return proxy.split('@')[1].split(':')[0] if '@' in proxy else proxy.split('://')[1].split(':')[0]
